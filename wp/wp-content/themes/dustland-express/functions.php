 ...
 */
define( 'DUSTLANDEXPRESS_THEME_VERSION' , '1.0.6' );
...
function dustlandexpress_theme_setup() {
	 ...
	 * to change 'dustlandexpress' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'dustlandexpress', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'dustlandexpress' ),
        'footer-bar' => __( 'Footer Bar Menu', 'dustlandexpress' )
	) );
...
function dustlandexpress_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'dustlandexpress' ),
		'id'            => 'sidebar-1',
		'description'   => '',
	...
	register_sidebar(array(
		'name' => __( 'Dustland Express Footer', 'dustlandexpress' ),
		'id' => 'dustlandexpress-site-footer',
        'description' => __( 'The footer will divide into however many widgets are put here.', 'dustlandexpress' )
	));
}
...
function dustlandexpress_theme_scripts() {
	...
	wp_enqueue_script( 'dustlandexpress-navigation', get_template_directory_uri() . '/js/navigation.js', array(), DUSTLANDEXPRESS_THEME_VERSION, true );
...
add_action('admin_notices', 'dustlandexpress_recommended_plugin_notice');

function dustlandexpress_recommended_plugin_notice() {
    global $pagenow;
    global $current_user;
    ...
    if ( $pagenow == 'plugins.php' ) {
	    if ( ! get_user_meta( $user_id, 'dustlandexpress_recommended_plugin_ignore_notice' ) ) {
	        echo '<div class="updated"><p>';
            printf( __('<p>Install the plugins we at <a href="http://www.kairaweb.com/" target="_blank">Kaira</a> recommended | <a href="%1$s">Hide Notice</a></p>'), '?dustlandexpress_recommended_plugin_nag_ignore=0' ); ?>
            <a href="<?php echo admin_url('plugin-install.php?tab=favorites&user=kaira'); ?>"><?php printf( __( 'WooCommerce', 'dustlandexpress' ), 'WordPress' ); ?></a><br />
            <a href="<?php echo admin_url('plugin-install.php?tab=favorites&user=kaira'); ?>"><?php printf( __( 'SiteOrigin\'s Page Builder', 'dustlandexpress' ), 'WordPress' ); ?></a><br />
            <a href="<?php echo admin_url('plugin-install.php?tab=favorites&user=kaira'); ?>"><?php printf( __( 'Meta Slider', 'dustlandexpress' ), 'WordPress' ); ?></a><br />
            <a href="<?php echo admin_url('plugin-install.php?tab=favorites&user=kaira'); ?>"><?php printf( __( 'Breadcrumb NavXT', 'dustlandexpress' ), 'WordPress' ); ?></a>
            <?php
	        echo "</p></div>";
	    }
	}
}
add_action('admin_init', 'dustlandexpress_recommended_plugin_nag_ignore');

function dustlandexpress_recommended_plugin_nag_ignore() {
    global $current_user;
    $user_id = $current_user->ID;
        
    /* If user clicks to ignore the notice, add that to their user meta */
    if ( isset($_GET['dustlandexpress_recommended_plugin_nag_ignore']) && '0' == $_GET['dustlandexpress_recommended_plugin_nag_ignore'] ) {
        add_user_meta( $user_id, 'dustlandexpress_recommended_plugin_ignore_notice', 'true', true );
    }
}
...
function dustlandexpress_premium_admin_menu() {
    ...
    $dustlandexpress_upgrade_page = add_theme_page( __( 'Dustland Express Premium', 'dustlandexpress' ), __( 'Dustland Express Premium', 'dustlandexpress' ), 'edit_theme_options', 'premium_upgrade', 'dustlandexpress_upgrade_page_render' );
}
 ...
 */
function dustlandexpress_upgrade_page_render() {
    locate_template( 'upgrade/kaira-upgrade-page.php', true, false );
}

/**
 * Enqueue Dustland Express admin stylesheet only on upgrade page.
 */
function dustlandexpress_load_admin_style($hook) {
    global $dustlandexpress_upgrade_page;
 
    if( $hook != $dustlandexpress_upgrade_page ) 
        return;
    
    wp_enqueue_style( 'dustlandexpress-upgrade-css', get_template_directory_uri() . '/upgrade/css/kaira-admin.css' );
}    
add_action( 'admin_enqueue_scripts', 'dustlandexpress_load_admin_style' );