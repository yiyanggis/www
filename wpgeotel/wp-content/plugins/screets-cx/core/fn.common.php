...
function cx_get_jquery_plug_opts() {
	...
	if( is_home() || is_front_page() ) {
		...
		// Hide on homepage
		if ( empty( $opts['display-chatbox-group']['always_display_on_homepage'] ) && !$display_chatbox )
			$render = false;
		else
			$render = true;
	...
	return apply_filters( 'cx_plugin_opts', array(
			'app_id'				=> $app_id,
			'auth_token'			=> $CX->auth_token,
			'render'				=> $render,
			'display_login'			=> ( !empty( $opts['display_login'] ) ) ? 1 : 0,
			...
			'msg'					=> array(
				'online' => addslashes($opts['when_online']), 
				'offline' => addslashes($opts['when_offline']),
				'prechat_msg' => html_entity_decode( $opts['prechat_msg'] ),
				'welc_msg' => html_entity_decode( $opts['welc_msg'] ),
				'waiting' => addslashes(__( 'Waiting', 'cx' )),
				'start_chat' => addslashes(__( 'Start Chat', 'cx' )),

				'offline_body' => html_entity_decode( $opts['offline_body'] ),
				'reply_ph' => addslashes($opts['popup_reply_ph']),
				'send_btn' => addslashes($opts['f_send_btn']),
				'no_op' => addslashes(__( 'No operators online', 'cx' )),
				'no_msg' => addslashes(__( 'No messages found', 'cx' )),
				'sending' => addslashes(__( 'Sending', 'cx' )),
				'connecting' => addslashes(__( 'Connecting', 'cx' )),
				'writing' => addslashes(__( '%s is writing', 'cx' )),
				'please_wait' => addslashes(__( 'Please wait', 'cx' )),
				'chat_online' => addslashes(__( 'Chat Online', 'cx' )),
				'chat_offline' => addslashes(__( 'Chat Offline', 'cx' )),
				'optional' => addslashes(__( 'Optional', 'cx' )),
				'your_msg' => addslashes(__( 'Your message', 'cx' )),
				'end_chat' => addslashes(__( 'End chat', 'cx' )),
				'conn_err' => addslashes(__( 'Connecting error!', 'cx' )),
				'chat_ended' => addslashes($opts['prechat_msg_ended_chat']),
				'field_empty' => addslashes(__( 'Please fill out all required fields', 'cx' )),
				'invalid_email' => addslashes(__( 'E-mail is invalid', 'cx' )),
				'you' => addslashes(__( 'You', 'cx' )),
				'online_btn' => addslashes(__( 'Online', 'cx' )),
				'offline_btn' => addslashes(__( 'Offline', 'cx' )),
				'op_not_allowed' => addslashes(__( 'Operators do not chat from here, only visitors. If you want to test chat box, you will want to use two different browsers or computers', 'cx' )),
				...
				'time' => array(
					'suffix' => addslashes(__( 'ago', 'cx' )),
			        'seconds' => addslashes(__( 'less than a minute', 'cx' )),
			        'minute' => addslashes(__( 'about a minute', 'cx' )),
			        'minutes' => addslashes(__( '%d minutes', 'cx' )),
			        'hour' => addslashes(__( 'about an hour', 'cx' )),
			        'hours' => addslashes(__( 'about %d hours', 'cx' )),
			        'day' => addslashes(__( 'a day', 'cx' )),
			        'days' => addslashes(__( '%d days', 'cx' )),
			        'month' => addslashes(__( 'about a month', 'cx' )),
			        'months' => addslashes(__( '%d months', 'cx' )),
			        'year' => addslashes(__( 'about a year', 'cx' )),
			        'years' => addslashes(__( '%d years', 'cx' ))
				)
			),
