...
class CX {
	...
	function init() {
		...
		// Shortcodes
		add_shortcode( 'chat_online', 'cx_shortcode_online' );
		add_shortcode( 'chat_offline', 'cx_shortcode_offline' );
	...
	function WPML( $opts_set ) {
		...
		foreach( $opts_set as $opt ) {
			...
			if( !empty( $opt['translate'] ) ) {
				...
				icl_register_string( 'Screets CX', $opt['name'], $opt['std'] );
	...
	function frontend_scripts() {
		...
		wp_register_script(
			'jquery-autosize', 
			plugins_url( 'assets/js/lib/jquery.autosize.min.js', __FILE__, array( 'jquery' ) ),
			null,
			'1.17.1',
			true
		);
	...
	function backend_scripts() {
		...
		if( $page == 'chat_x'  ) {
			...
			wp_register_script( 
				'jquery-tipsy', 
				plugins_url( 'assets/js/lib/jquery.tipsy.min.js', __FILE__, array( 'jquery' ) ),
				null,
				'1.0'
			);
			...
			wp_register_script( 
				'cx-console', 
				plugins_url( 'assets/js/cx.console.js', __FILE__, array( 'jquery' ) ),
				null,
				CX_VERSION
			);
	...
	function update() {
		...
		if( defined( 'CX_ACTIVATE_K' ) ) return;
	...
	public function auth() {
		...
		$this->auth_token = $token_gen->createToken( $data, $opts );
	...
	function load_app_js() {
		...
		wp_register_script( 
			'firebase', 
			plugins_url( 'assets/js/firebase.js', __FILE__ ),
			null,
			null,
			true
		);
		...
		wp_enqueue_script( 'firebase-simple-login' );*/
		...
		wp_register_script(
			'cx-app',
			plugins_url( 'assets/js/cx.app' . $suffix_js . '.js', __FILE__, array( 'jquery' ) ),
			null,
			CX_VERSION,
			true
		);
		...
		$ajax_vars = array(
			...
			'current_page'		=> cx_current_page_url(),
			'ip' 				=> cx_ip_address()
		);
		...
		if( is_user_logged_in() ) {
			...
			$ajax_vars['user_id'] = $user_prefix . $this->user->ID;
			$ajax_vars['user_name'] = cx_get_operator_name();
			$ajax_vars['user_email'] = $this->user->user_email;
			$ajax_vars['user_email_hash'] = md5( $this->user->user_email );
		...
		if( defined( 'CX_OP' ) ) {
			...
			$ajax_vars['app_id'] = !empty( $this->opts['app_url'] ) ? $this->opts['app_url'] : null;
			$ajax_vars['auth_token'] = ( !empty( $this->auth_token ) ) ? $this->auth_token : null;
		...
		wp_localize_script( 'cx-app', 'cx', $ajax_vars );
	}
}
