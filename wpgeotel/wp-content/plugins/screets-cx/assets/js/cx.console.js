	...
	$(document).ready(function() {
		...
		$('body').cx({
			...
			user_online : function( user ) {
				...
				if( user.id != this.data.user.id ) { // If operator didn't logged out itself

					// Play sound
					this.play_sound( 'cx-online' );

					// Notify user
					this.notify( cx.msgs.new_user_online, user.name + ' (' + user.type + ')' );
			...
			new_msg : function( msg ) {
				...
				if( !msg.old_msg && !msg.first_load && msg.user_id != this.data.user.id ) {
					...
					// Notify user
					this.notify( cx.msgs.new_msg, msg.name + ': ' + msg.msg );
				...
				$( document ).on( 'click', '#CX_users li', function() {
					...
					self.get_user_data( $(this).data( 'id' ), function( user ) {
						...
						if( user.current_page )
							user_html = user_html + '<div id="CX_popup_info_0"><strong><i class="cx-ico-preview"></i> Active Page:</strong><a id="CX_active_page" href="' + user.current_page + '" target="_blank"><span>' + user.current_page + 'x</span></a></div>';
