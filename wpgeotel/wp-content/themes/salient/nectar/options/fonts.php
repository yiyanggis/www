	...
	if(!empty($options[$location.'_style']) ) {
		...
		if(!in_array($font.'-'.$weight, $enqueued_fonts_with_weight)){
			...
			if(!empty($options['include-latin-chars']) && $options['include-latin-chars'] == 1 ) {
				wp_enqueue_style( "options_typography_$font-$weight", "http://fonts.googleapis.com/css?family=$font:$weight&subset=latin,latin-ext", false, null, 'all' );
			}
			else {
				wp_enqueue_style( "options_typography_$font-$weight", "http://fonts.googleapis.com/css?family=$font:$weight", false, null, 'all' );
			}
		}// in array check
