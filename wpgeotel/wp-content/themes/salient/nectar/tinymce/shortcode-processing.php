...
function nectar_image_with_animation($atts, $content = null) { 
    extract(shortcode_atts(array("animation" => 'Fade In From Left', "image_url" => '', 'alt' => ''), $atts));
	
	$parsed_animation = str_replace(" ","-",$animation);
	(!empty($alt)) ? $alt_tag = $alt : $alt_tag = null;
    return'<img class="img-with-animation" data-animation="'.strtolower($parsed_animation).'" src="'.$image_url.'" alt="'.$alt_tag.'" />';
}
...
function nectar_testimonial_slider($atts, $content = null) { 
    extract(shortcode_atts(array("animation" => 'Fade In From Left', "image_url" => ''), $atts));
	
	
    return '<div class="col span_12 testimonial_slider"><div class="slides">'.do_shortcode($content).'</div></div>';
}
...
function nectar_divider($atts, $content = null) {  
    extract(shortcode_atts(array("line" => 'false', "line_type" => 'No Line'), $atts));
	
	if($line_type == 'Small Thick Line'){
		$divider = '<div class="divider-thick-border"></div>';
...
function nectar_carousel($atts, $content = null) {  
	...
	<div class="carousel-wrap"></span><ul class="row carousel" data-scroll-speed="' . $scroll_speed . '" data-easing="' . $easing . '">';
