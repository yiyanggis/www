YY.Image=YY.Class(
{
	...
	setDetailInfo=function(domid, googlePlaceFinder){
		var thumbnail=$("#"+domid);
	...
	getPlaceID=function(results,status){
		...
		if(results.length>0)
		{
			...
			if(results.length>3){
				...
				for(var i=0;i<n;i++){
					...
					this.googlePlaceFinder.getDetails(this.placeID, this.placeServiceCallback);
	...
	placeServiceCallback:function(place, status) {
	...
	// function(url, marker, date, desc,addr, googlePlaceFinder){
	// this.imageURL=url;
	// this.marker=marker;
	// this.date=date;
	// this.desc=desc;
