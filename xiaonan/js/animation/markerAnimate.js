...
google.maps.Polyline.prototype.animateTo = function(newPosition, options) {
  defaultOptions = {
    duration: 1000,
    easing: 'linear',
    complete: null
  ...
  var lastPt=path.j[path.length-1];

  this.AT_startPosition_lat = lastPt.lat();
  this.AT_startPosition_lng = lastPt.lng();
  var newPosition_lat = newPosition.lat();
  var newPosition_lng = newPosition.lng();
  ...
  var animateStep = function(polyline,newPt, ratio) {            
    //var ellapsedTime = (new Date()).getTime() - startTime;
    //var durationRatio = ellapsedTime / options.duration; // 0 - 1
    var ellapsedTime=100;
    var durationRatio=0.1;
    var easingDurationRatio = ratio;

    var path=polyline.getPath();
    //var marker=path[path.length-1];
    ...
    if (ratio < 1) {
      var deltaPosition = new google.maps.LatLng( polyline.AT_startPosition_lat + (newPosition_lat - polyline.AT_startPosition_lat)*easingDurationRatio,
                                                  polyline.AT_startPosition_lng + (newPosition_lng - polyline.AT_startPosition_lng)*easingDurationRatio);
      //marker.setPosition(deltaPosition);
      //newPt=new google.maps.LatLng()
      //path.pop();
      path.push(deltaPosition);

      // use requestAnimationFrame if it exists on this browser. If not, use setTimeout with ~60 fps
      //if (window.requestAnimationFrame) {
      //  polyline.AT_animationHandler = window.requestAnimationFrame(function() {animateStep(polyline, ratio)});                
      //} else {
        //polyline.AT_animationHandler =  
      //}

      ratio+=0.1;
      console.log(ratio);
      setTimeout(function() {animateStep(polyline,newPt, ratio)}, 100);
  ...
  animateStep(this, newPosition, ratio);
}
